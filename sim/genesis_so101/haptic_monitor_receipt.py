#!/usr/bin/env python3
"""Seal the operator judgment for a single-arm monitor-only stage."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--mapping-verdict", choices=MONITOR_MAPPING_CHOICES, required=True
    )
    parser.add_argument("--leader-moves-freely-after-monitor", action="store_true")
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    report = build_single_arm_monitor_receipt(
        ...
        leader_moves_freely_after_monitor=(
            args.leader_moves_freely_after_monitor
        ),
    )
    ...
    if not report["accepted"]:
        raise RuntimeError("monitor receipt was recorded but not accepted")

#!/usr/bin/env python3
"""Send a bounded synthetic five-joint contact signal for the arm bench gate."""
...
def main() -> None:
    ...
    parser.add_argument("--duration-s", type=float, default=5.5)
    ...
    if not 5.0 <= args.duration_s <= 7.0:
        raise ValueError("arm bench sender duration must be between 5 and 7 seconds")
    if not 20.0 <= args.hz <= 30.0:
        raise ValueError("arm bench sender rate must be between 20 and 30 Hz")
    if not 0.5 <= args.contact_force_n <= 5.0:
        raise ValueError("arm bench contact force must be between 0.5 and 5 N")
    if not 0.0 < abs(args.reaction_effort) <= 3.35:
        raise ValueError("arm bench reaction effort must be in (0, 3.35]")

    side_offset = 0 if args.side == "left" else len(ARM_MOTORS)
    efforts = [0.0] * 12
    for motor_index in range(len(BENCH_MOTORS)):
        efforts[side_offset + motor_index] = args.reaction_effort
    forces = (
    ...
    report = {
        ...
        "simultaneous_selected_channels": len(BENCH_MOTORS),

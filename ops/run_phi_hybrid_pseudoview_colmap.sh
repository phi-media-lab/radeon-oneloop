...
usage() {
  printf 'usage: %s <four-view> <texture> <conditioning> <observed-initialization> <proposal> <audit> <review-json> <output-root> <python-bin>\n' "$0" >&2
  exit 64
}
...
run_id="hybrid_pseudoview_colmap_$(date -u +%Y%m%dT%H%M%SZ)_${BASHPID}"
run_dir="$output_root/$run_id"

mkdir -p "$output_root"
[[ ! -e "$run_dir" ]]
[[ -d "$four_view" && -d "$texture" && -d "$conditioning" && -d "$observed_initialization" && -d "$proposal" && -d "$audit" ]]
...
printf 'Hybrid pseudo-view COLMAP dataset complete: %s\n' "$run_dir"

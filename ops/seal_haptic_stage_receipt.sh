...
if [[ $# -ne 2 ]]; then
  printf 'usage: %s SOURCE_HAPTIC_RUN_DIR PERCEPTION\n' "$0" >&2
...
receipt_id="$(date -u +%Y%m%dT%H%M%SZ)_${BASHPID}_single_joint_haptic_receipt"
...
printf '%s\n' \
  'schema_version: radeon_oneloop.haptic_stage_receipt_run.v1' \
  'formal: false' \
...
"$python_bin" -m sim.genesis_so101.haptic_stage_receipt \
...
printf '{"status":"done_single_joint_stage_accepted"}\n' >"$receipt_dir/DONE"
trap - EXIT
printf 'haptic stage receipt sealed: %s\n' "$receipt_dir"

...
if [[ $# -ne 5 ]]; then
  printf 'usage: %s LEFT_PORT RIGHT_PORT LEFT_ID RIGHT_ID ARM_RECEIPT_RUN_DIR\n' "$0" >&2
  exit 64
fi
...
duration_s=${ONELOOP_HAPTIC_DUAL_MONITOR_DURATION_S:-40}
timeout_s=${ONELOOP_HAPTIC_MONITOR_TIMEOUT_S:-240}
leader_port=${ONELOOP_LIVE_PORT:-58081}
feedback_port=${ONELOOP_HAPTIC_PORT:-58082}
run_root=${ONELOOP_HAPTIC_DUAL_MONITOR_RUN_ROOT:-$repo_root/runs/haptic_dual_monitor}
...
run_id="$(date -u +%Y%m%dT%H%M%SZ)_${BASHPID}_amd_dual_arm_monitor"
...
if not math.isfinite(value) or not 30.0 <= value <= 60.0:
    raise SystemExit("dual monitor duration must be between 30 and 60 seconds")
PY
...
write_hashes() {
  ...
  for name in authorization.json consumer.log publisher.log consumer/READY.json \
    consumer/metrics.json publisher_metrics.json consumer/live_front_cam.png \
    consumer/live_hand_cam.png gate.json; do
...
cleanup() {
  ...
  if [[ $status -ne 0 && ! -e "$run_dir/DONE" ]]; then
    write_hashes
    printf '{"status":"failed_dual_arm_monitor","exit_code":%d}\n' \
...
printf '%s\n' \
  'schema_version: radeon_oneloop.amd_haptic_dual_monitor_run.v1' \
  'formal: false' \
  "stage: $stage" \
...
while [[ ! -f "$ready_file" ]]; do
  if ! kill -0 "$consumer_pid" 2>/dev/null || \
     ! kill -0 "$publisher_pid" 2>/dev/null; then
    printf '%s\n' 'dual monitor process exited before readiness' >&2
    exit 70
  fi
  if (( SECONDS >= ready_deadline )); then
    printf '%s\n' 'dual monitor stage did not become ready within 180 seconds' >&2
    exit 70
  fi
  sleep 0.2
done
printf '%s\n' 'READY: within 5 seconds, exercise all six channels on both arms; verify same-side parallel motion'
...
"$lerobot_python" -m sim.genesis_so101.haptic_dual_monitor_gate \
...
printf '{"status":"done_dual_arm_monitor_machine_accepted"}\n' \
  >"$run_dir/DONE"
trap - EXIT INT TERM
printf 'dual-arm monitor gate passed: %s\n' "$run_dir"

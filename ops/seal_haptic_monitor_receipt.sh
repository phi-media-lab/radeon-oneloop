...
if [[ $# -ne 2 ]]; then
  printf 'usage: %s SOURCE_MONITOR_RUN_DIR MAPPING_VERDICT\n' "$0" >&2
  printf 'mapping: correct_same_side_same_direction|wrong_side_or_direction|unstable_or_unusable\n' >&2
  exit 64
fi
...
receipt_id="$(date -u +%Y%m%dT%H%M%SZ)_${BASHPID}_single_arm_monitor_receipt"
...
case "$mapping_verdict" in
  correct_same_side_same_direction|wrong_side_or_direction|unstable_or_unusable) ;;
  *) exit 64 ;;
esac
mkdir -p "$receipt_dir"
...
printf '%s\n' \
  'schema_version: radeon_oneloop.haptic_monitor_receipt_run.v1' \
  'formal: false' \
...
"$python_bin" -m sim.genesis_so101.haptic_monitor_receipt \
  ...
  --leader-moves-freely-after-monitor \
...
printf '{"status":"done_single_arm_monitor_stage_accepted"}\n' \
  >"$receipt_dir/DONE"
trap - EXIT
printf 'single-arm monitor receipt sealed: %s\n' "$receipt_dir"

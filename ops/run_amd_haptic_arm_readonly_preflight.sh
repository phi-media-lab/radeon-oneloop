...
run_root=${ONELOOP_HAPTIC_ARM_PREFLIGHT_RUN_ROOT:-$repo_root/runs/haptic_arm_readonly_preflight}
...
run_id="$(date -u +%Y%m%dT%H%M%SZ)_${BASHPID}_amd_haptic_arm_readonly_preflight"
...
printf '%s\n' \
  'schema_version: radeon_oneloop.amd_haptic_arm_readonly_preflight_run.v1' \
  'formal: false' \
  ...
  'max_torque_limit_raw_candidate: 20' \
  'max_position_offset_deg_candidate: 0.5' \
...
printf '{"status":"done_arm_read_only_no_physical_output"}\n' \
  >"$run_dir/DONE"
trap - EXIT INT TERM
printf 'single-arm read-only preflight passed: %s\n' "$run_dir"

...
run_id="$(date -u +%Y%m%dT%H%M%SZ)_${BASHPID}_amd_gaussian_static_gate"
...
printf '%s\n' \
  'schema_version: radeon_oneloop.amd_gaussian_static_gate_run.v1' \
  'formal: false' \
  'host_role: amd_apu_nonformal_runtime_integration' \
  'renderer: Genesis_amdgpu_plus_VkSplat_RADV' \
...
timeout --signal=TERM --kill-after=15 300 \
  "$python_bin" -m sim.genesis_so101.gaussian_genesis_static \
...
printf '{"status":"done_nonformal_static_gate"}\n' >"$run_dir/DONE"
trap - EXIT INT TERM
printf 'AMD Genesis Gaussian static gate passed: %s\n' "$run_dir"

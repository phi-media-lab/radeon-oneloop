...
run_id="$(date -u +%Y%m%dT%H%M%SZ)_${BASHPID}_amd_haptic_readonly_preflight"
...
cleanup() {
  ...
  if [[ $status -ne 0 && ! -e "$run_dir/DONE" ]]; then
    ...
    for path in metrics.json stdout.log stderr.log; do
...
printf '%s\n' \
  'schema_version: radeon_oneloop.amd_haptic_readonly_preflight_run.v1' \
  'formal: false' \
  'physical_output_commands: false' \
  'serial_register_writes: 0' \
  "side: $side" \
  "motor: $motor" \
  'max_torque_limit_raw_candidate: 30' \
  'max_position_offset_deg_candidate: 1.0' \
...
printf '{"status":"done_read_only_no_physical_output"}\n' >"$run_dir/DONE"
trap - EXIT INT TERM
printf 'haptic read-only preflight passed: %s\n' "$run_dir"
